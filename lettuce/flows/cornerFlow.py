...
class CornerFlow:
    ...
    def __init__(self, shape, re, ma, lattice, domain_constraints, domain_length_x_lu, domain_length_x_pu,
                  char_velocity_pu, reference_height_pu,  # ref. height can be outside the domain and mimic the house-eg_height...
                  corner_position_lu: tuple,
                  ground_height_pu: float,
                  u_init: int = 0,
                 char_density_pu=1,
                  inlet_bc: str = "equin", outlet_bc: str = "eqoutp",
                  inlet_ramp_steps = 0,
                  lateral_bc: str = "slip", top_bc: str = "slip",
                  ground_bc: str = "fwbb",
                  corner_bc: str = "fwbb",
                  corner_solid_boundary_data=None,
                  ground_solid_boundary_data=None,  # will be None for combine_solids == True
                  K_Factor=10,  # K_factor for SEI boundary inlet
                  L=3,  # L for SEI
                  N=34,  # N number of random voctices for SEI
                  wsp_shift_up_pu=0,  # how many PU to shift the u_inlet profile upwards, without altering the ground_height
                  wsp_y0=None,
                  wsp_alpha=0.25,
                  ):
        ...
        # solid_boundary_data
        self.corner_solid_boundary_data = corner_solid_boundary_data
        ...
        self.units = UnitConversion(
            ...
            reynolds_number=re,
            mach_number=ma,
            characteristic_length_lu=self.char_length_pu*res,
        ...
        self.corner_mask = np.zeros(shape=self.shape, dtype=bool)

        if self.corner_solid_boundary_data is not None:
            self.corner_mask = self.corner_solid_boundary_data.solid_mask
    ...
    def boundaries(self):
        ...
        # INLET, OUTLET, TOP/HEAVEN, BOTTOM/GROUND, corner/SOLID
        ...
        if self.inlet_bc.casefold() == 'eqin':
            ...
            inlet_boundary_condition = NonEquilibriumExtrapolationInletU(self.units.lattice, self.units, [-1, 0, 0],
                                                                         np.array(self.initial_solution(self.grid)[
                                                                                      1]))  # original aus der Arbeit, Übergibt die y-Koordinaten aller grid-Punkte und erzeugt so die von y abhängige initial solution für das komplette Feld. Wird für MLKs Rechnung in Feld-Größe benötigt
        ...
        if self.inlet_bc.casefold() == 'rampeqin':
            inlet_boundary_condition = RampedEquilibriumBoundaryPU(self.in_mask, self.units.lattice, self.units,
                                                                   u_inlet, ramp_steps=self.inlet_ramp_steps)
        else:
        ...
        if self.outlet_bc.casefold() == 'eqoutp':
            outlet_boundary_condition = EquilibriumOutletP(self.units.lattice, [1, 0, 0],
                                                           rho0=self.units.convert_pressure_pu_to_density_lu(0))
        elif self.outlet_bc.casefold() == 'eqoutu':
            print("(INFO) Equilibrium Outlet U (!) was selected.")
            out_mask = np.zeros_like(self.solid_mask)
            out_mask[-1, 1:, :] = True
            out_mask = out_mask & ~self.solid_mask  # outlet where no solid and at the rear end of domain
        ...
        elif self.top_bc.casefold() == 'slip':
            top_mask = np.zeros_like(self.solid_mask)
            top_mask[:, -1, :] = True
            top_mask = top_mask & ~self.solid_mask
            top_boundary_condition = SlipBoundary(top_mask, self.units.lattice, 1)
        else:
            top_mask = np.zeros_like(self.solid_mask)
            top_mask[:, -1, :] = True
            top_mask = top_mask & ~self.solid_mask
            top_boundary_condition = EquilibriumBoundaryPU(top_mask, self.units.lattice, self.units,
                                                           self.initial_solution(self.grid)[0])

        # LATERAL
        print("initializing lateral boundary condition...")
        if self.lateral_bc.casefold() == 'zgo':
            lateral_boundary_condition = ZeroGradientOutlet(self.units.lattice, [0, 1, 0])
        elif self.lateral_bc.casefold() == 'slip':
            lateral_mask = np.zeros_like(self.solid_mask)
            lateral_mask[:, :, [0, -1]] = True
            lateral_mask = lateral_mask & ~self.solid_mask
            lateral_boundary_condition = SlipBoundary(lateral_mask, self.units.lattice, 2)
        else:
            lateral_mask = np.zeros_like(self.solid_mask)
            lateral_mask[:, :, [0, -1]] = True
            lateral_mask = lateral_mask & ~self.solid_mask
            lateral_boundary_condition = EquilibriumBoundaryPU(lateral_mask, self.units.lattice, self.units,
                                                               self.initial_solution(self.grid)[0])
        ...
        if self.ground_solid_boundary_data is not None:  # ground as IBB
            if self.ground_bc.casefold() == 'ibb':
                ground_boundary_condition = InterpolatedBounceBackBoundary_occ(
                    self.ground_solid_boundary_data.solid_mask, self.lattice,
                    solid_boundary_data=self.ground_solid_boundary_data)
            elif self.ground_bc.casefold() == 'hwbb':
                ground_boundary_condition = HalfwayBounceBackBoundary_occ(self.ground_solid_boundary_data.solid_mask,
                                                                          self.units.lattice,
                                                                          solid_boundary_data=self.ground_solid_boundary_data)
            elif self.ground_bc.casefold() == 'fwbb':
                ground_boundary_condition = FullwayBounceBackBoundary_occ(self.ground_solid_boundary_data.solid_mask,
                                                                          self.units.lattice,
                                                                          solid_boundary_data=self.ground_solid_boundary_data,
                                                                          global_solid_mask=self.solid_mask,
                                                                          periodicity=(False, False, False))
            else:  # default to basic full-mask (fullway) BounceBack
                print("(!) ground boundary specification could not be interpreted! Using basic BB from lettuce 0.2.2!")
                ground_boundary_condition = BounceBackBoundary(self.ground_solid_boundary_data.solid_mask,
                                                               self.units.lattice)

        else:
            print(
                f"(INFO) no ground_solid_boundary_data available to flow.boundaries but ground_bc appears to be wanted. Using self.ground_mask[:, 0, :] = True as ground_mask")
            ...
            if self.ground_bc is not None:  # if ground BC is specified for regular SBB
                if self.ground_bc.casefold() == 'hwbb':
                    ground_boundary_condition = HalfwayBounceBackBoundary_occ(self.ground_mask, self.units.lattice,
                                                                              periodicity=(False, False, False))
                elif self.ground_bc.casefold() == 'fwbb':
                    ground_boundary_condition = FullwayBounceBackBoundary_occ(self.ground_mask, self.units.lattice,
                                                                              global_solid_mask=self.solid_mask,
                                                                              periodicity=(False, False, False))
                else:  # default to basic full-mask (fullway) BounceBack
        ...
        # corner
        print("initializing corner boundary condition...")
        corner_boundary_condition = None
        if self.corner_solid_boundary_data is not None:
            if self.corner_bc.casefold() == 'fwbb':
                corner_boundary_condition = FullwayBounceBackBoundary_occ(self.corner_solid_boundary_data.solid_mask,
                                                                         self.lattice, self.corner_solid_boundary_data,
                                                                         global_solid_mask=self.solid_mask,
                                                                         periodicity=(False, False, False))
            elif self.corner_bc.casefold() == 'hwbb':
                corner_boundary_condition = HalfwayBounceBackBoundary_occ(self.corner_solid_boundary_data.solid_mask,
                                                                         self.lattice, self.corner_solid_boundary_data,
                                                                         periodicity=(False, False, False))
                pass
            elif self.corner_bc.casefold() == 'ibb' or 'ibb1':
                corner_boundary_condition = InterpolatedBounceBackBoundary_occ(self.corner_solid_boundary_data.solid_mask,
                                                                              self.lattice,
                                                                              self.corner_solid_boundary_data)
            else:
                corner_boundary_condition = BounceBackBoundary(self.corner_solid_boundary_data.solid_mask, self.lattice)
        else:
            print("(!) flow.boundary(): corner_solid_boundary_data is None! no corner boundary created!")
        ...
        if (ground_boundary_condition is not None) and (corner_boundary_condition is not None):
            print("INFO: flow.boundaries contains SEPERATE corner and ground solid boundaries")
            boundaries = [
                ...
                corner_boundary_condition
            ]
        elif corner_boundary_condition is not None:  # if there is no ground_boundary_condition use only one solid_boundary, which is in corner_BC
            print("INFO: flow.boundaries contains COMBINED corner and ground solid boundaries")
            boundaries = [
                inlet_boundary_condition,
                outlet_boundary_condition,
                top_boundary_condition,
        ...
        elif ground_boundary_condition is not None:  # if there is no corner_boundary_condition use only one solid_boundary, which is in ground_BC
        ...
        for boundary in boundaries:
            if hasattr(boundary, 'f_index'):
                ...
                if boundary.f_index.shape[0] > 0:
                    boundary.f_index = boundary.f_index[torch.where(
                        ~self.lattice.convert_to_tensor(self.non_free_flow_mask)[
                            boundary.f_index[:, 1], boundary.f_index[:, 2], boundary.f_index[:, 3] if len(
                                self.shape) == 3 else None])]
            ...
            if hasattr(boundary, 'f_index_fwbb'):
                ...
                print(
                    f"boundary {boundary} has f_index_fwbb with {num_entries} entries, but is FWBB so no cleanup possible. Provide global_solid_mask to FWBB-initialization, if you need cleanup of f_index_fwbb")
            ...
            if hasattr(boundary, 'f_index_gt'):
                ...
                if boundary.f_index_gt.shape[0] > 0:
                    boundary.d_gt = boundary.d_gt[torch.where(~self.lattice.convert_to_tensor(self.non_free_flow_mask)[
                        boundary.f_index_gt[:, 1], boundary.f_index_gt[:, 2], boundary.f_index_gt[:, 3] if len(
                            self.shape) == 3 else None])]
            ...
            if hasattr(boundary, 'f_index_lt'):
                ...
                if boundary.f_index_lt.shape[0] > 0:
                    boundary.d_lt = boundary.d_lt[torch.where(~self.lattice.convert_to_tensor(self.non_free_flow_mask)[
                        boundary.f_index_lt[:, 1], boundary.f_index_lt[:, 2], boundary.f_index_lt[:, 3] if len(
                            self.shape) == 3 else None])]
    ...
    def overlap_all_solid_masks(self):
        ...
        self._solid_mask = self.solid_mask | self.corner_mask | self.ground_mask
    ...
    def calculate_non_free_flow_mask(self):
        ...
        out_mask = np.zeros_like(self.solid_mask)
        out_mask[-1, 1:, :] = True
        out_mask = out_mask & ~self.solid_mask
        ...
        #self._non_free_flow_mask = self.solid_mask | self.corner_mask | self.ground_mask | out_mask | top_mask | lateral_mask | self.in_mask
        self._non_free_flow_mask = self.solid_mask | self.corner_mask | self.ground_mask | out_mask | self.in_mask
        time1 = time.time() - time0
    ...
    def wind_speed_profile_log_law(self, y, u_0, y_min=1.2, y_0=0.02, kat=2):
        ...
        z_0_kat2 = 0.05
        k_r = 0.19 * (
                    y_0 / z_0_kat2) ** 0.07  # hier k_r ~ 0.1782, das ist zw. Kapt. I und II // 0.05 ist z_0,II für Kat.2

        ##(MK_torch-Version): return torch.where(y < y_min, u_0 * k_r * torch.log(torch.tensor(y_min / y_0, device=self.units.lattice.device)) * 1, u_0 * k_r * torch.log(y / y_0) * 1)
        return np.where(y < y_min,
                        u_0 * k_r * np.log(y_min / y_0),  # const. unterhalb der Mindeshöhe
                        u_0 * k_r * np.log(y / y_0)
                        # log-Profil oberhalb der Mindeshöhe (streng genommen bis zur max-Höhe)
                        )
    ...
    def reyolds_stress_tensor(self, z, u_0):
        ...
        """
        corner_length_pu = 10
        # roof_height_pu = self.units.convert_length_to_pu(self.eg_height)  # corner_length_pu * 1.25

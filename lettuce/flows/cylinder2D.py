...
class Cylinder2D:
    ...
    def __init__(self, reynolds_number, mach_number, lattice, char_length_pu, char_length_lu, char_velocity_pu=1, y_lu=5, x_lu=10, lateral_walls='periodic', bc_type='ibb1', perturb_init=True, u_init=0, x_offset=0, y_offset=0, radius=0):
        ...
        if self.lateral_walls:  # if channel flow or slip-walls and not y-periodic flow
    ...
    def boundaries(self):
        ...
        if self.bc_type == 'hwbb' or self.bc_type == 'HWBB':
            ...
            obstacle_boundary = InterpolatedBounceBackBoundary(self.obstacle_mask, self.units.lattice, x_center=(self.y_lu/2 - 0.5), y_center=(self.y_lu/2 - 0.5), radius=self.radius)

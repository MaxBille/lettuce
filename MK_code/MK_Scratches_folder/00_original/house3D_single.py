...
flow = HouseFlow3D(350, 250, 200, Re, 0.1, lattice, 50, 50*50)
flow.mask, points = flow.house([2.5,  2.5, 0], 1, 1, 0.5, 1.15, 1.15, 45)
collision = lt.KBCCollision3D(lattice, tau=flow.units.relaxation_parameter_lu)
streaming = lt.StandardStreaming(lattice)
simulation = lt.Simulation(flow, lattice, collision, streaming, nan_steps=6000)
vtkrep = lt.VTKReporter(lattice, flow, interval=1000, filename_base=f"/scratch/mkliem3s/allperiodic/house_Re{Re}_", nan_out=True)
...
print('MLUPS: ', simulation.step(8 * 10**5))

time_end = time.time()

print("The simulation took {} hours.".format((time_end - time_start) / 3600))
for rep in observ:
    np.save(f"/scratch/mkliem3s/allperiodic/p_Re{Re}_{rep.observable.coordinates[0]}_{rep.observable.coordinates[1]}_{rep.observable.coordinates[2]}.npy", rep.out)
simulation.save_checkpoint("/scratch/mkliem3s/saves/Haus_single_Re{Re}_save")

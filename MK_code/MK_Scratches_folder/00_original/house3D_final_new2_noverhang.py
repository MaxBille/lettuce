...
class HouseFlow3D(object):
    ...
    def house2(self, o, eg_x_length, eg_y_length, roof_height, roof_overhang, roof_width=0, angle=45):
        ...
        #auf dem Dach:
        x = np.linspace(-0.1, 0, 7, endpoint=True)
...
np.save(f"/scratch/mkliem3s/final/p_final2_{identifier}_{angle}deg_Re{Re}.npy", observ.out)
np.save(f"/scratch/mkliem3s/final/mass_final2_{identifier}_{angle}deg_Re{Re}.npy", massrep.out)
simulation.save_checkpoint(f"/scratch/mkliem3s/saves/Haus_final2_{identifier}_{angle}deg_Re{Re}_save")

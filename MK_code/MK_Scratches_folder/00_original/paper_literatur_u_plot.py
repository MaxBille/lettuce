...
def calculate_errors_u():
    ...
    for a in range(0, 3):
        ...
        offset = [1, 0.55, 0, -0.55, -1]
        for i in range(1, 6):
            ref = np.genfromtxt(f'/home/martin/Ablage/p_auswertung/u_{2 * a + 3}10_{i}.csv', delimiter=',')
            ref[:, 0] = (ref[:, 0] + offset[i - 1]) * 4
            ref = ref[::-1, :]
            values = []
...
for h in range(0, 3):
    ...
    u1 = np.load(f"/home/martin/Ablage/u_auswertung/u_verify_RefVel_1_3_{txt3[h]}deg.npy")[100000:, ...]
    for i in range(1, 6):
        ref.append(np.genfromtxt(f'/home/martin/Ablage/p_auswertung/u_{txt2[h]}10_{i}.csv', delimiter=','))
    for i in range(0, 5):
        plt.plot(np.mean(u1[:, 0, i + 1, 1:] / u_0u, axis=0) / 4 + offset[i], z[1:], color="black")
    ...
    lines.insert(lines.index("]") + 1,
                            "\draw[draw=white] (1, 2.85) -- (1.25, 2.85) node[anchor=south] {$u_x/u_0$};\n" + \
                            "\draw[line width=0.4mm] (1.5, 2.65) -- (1, 2.65);\n"  + \
                            "\draw[line width=0.4mm] (1.5, 2.65) node[anchor=south] {\small 2.0} -- (1.5, 2.56);\n" + \
                            "\draw[line width=0.4mm] (1.25, 2.65) node[anchor=south] {\small 1.0} -- (1.25, 2.56);\n" + \
    ...
    text += end
    plt.show()
    with open(f"literatur_u_{int(txt[h]/10)}.tikz", "w") as f:
        f.write(text)
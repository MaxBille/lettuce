class InterpolatedBounceBackBoundary:
    ...
    def __call__(self, f, f_collided):
        ...
        if self.interpolation_order == 2:
            ...
            f_tmp = torch.where(self.d.to_dense() <= 0.5,  # if d<=1/2
                                2*self.d.to_dense()*f_collided.to_dense()+(1-2*self.d.to_dense())*f,  # interpolate from second fluid node
                                (1/(2*self.d.to_dense()))*f_collided.to_dense()+(1-1/(2*self.d.to_dense()))*f_collided.to_dense()[self.lattice.stencil.opposite])  # else: interpolate from opposing populations on x_b
    ...
    def calc_force_on_boundary(self, f_bounced, f_collided):
        ...
        tmp = torch.where(self.f_mask, f_collided.to_dense() + f_bounced[self.lattice.stencil.opposite], torch.zeros_like(f_bounced))  #RIGHT
...
class Simulation:
    ...
    def __init__(self, flow, lattice, collision, streaming):
        ...
        if self.store_f_collided:
            ...
            if self.lattice.D == 2:
                fc_q, fc_x, fc_y = torch.where(self._boundaries[-1].f_mask+self._boundaries[-1].f_mask[self.lattice.stencil.opposite])
                ...
                self.f_collided = deepcopy(torch.sparse_coo_tensor(indices=self.fc_index,
                                                          values=self.f[self.fc_index[0], self.fc_index[1], self.fc_index[2]],
                                                          size=self.f.size()))
            if self.lattice.D == 3:
                ...
                self.f_collided = deepcopy(torch.sparse_coo_tensor(indices=self.fc_index,
                                                          ...
                                                          size=self.f.size()))
            log_VRAM("SIMULATION_init_after_f_collided_toSparse")

...
#        self.d = self.lattice.convert_to_tensor(self.d)
    ...
    def __call__(self, f, f_collided):
        ...
        log_VRAM("IBB1_call_after_Bounce")
...
class Simulation:
    ...
    def __init__(self, flow, lattice, collision, streaming):
        ...
        if self.store_f_collided:
            self.f_collided = torch.zeros_like(self.f)
            log_VRAM("SIMULATION_init_after_f_collided_zeros")
            for j in range(self._boundaries[-1].d_sequential.shape[0]):
                # 0:q, 1:x, 2:y, 3:z 4:d
                q = int(self._boundaries[-1].d_sequential[j,0])
                x = int(self._boundaries[-1].d_sequential[j,1])
                y = int(self._boundaries[-1].d_sequential[j,2])
                z = int(self._boundaries[-1].d_sequential[j,3])
                self.f_collided[q,x,y,z] = copy(self.f[q,x,y,z])
            self.f_collided = self.f_collided.to_sparse()
        log_VRAM("SIMULATION_init_after_store_f_collided")
    ...
    def step(self, num_steps):
        ...
        for _ in range(num_steps):  # simulate num_step timesteps
            ...
            if self.store_f_collided:
                #self.f_collided = deepcopy(self.f)
                self.f_collided = torch.zeros_like(self.f)
                log_VRAM("SIMULATION_step_after_f_collided_zeros")
                for j in range(self._boundaries[-1].d_sequential.shape[0]):
                    # 0:q, 1:x, 2:y, 3:z 4:d
                    q = int(self._boundaries[-1].d_sequential[j,0])
                    x = int(self._boundaries[-1].d_sequential[j,1])
                    ...
                    z = int(self._boundaries[-1].d_sequential[j,3])
                    self.f_collided[q,x,y,z] = copy(self.f[q,x,y,z])
                log_VRAM("SIMULATION_step_after_copy_f")
                self.f_collided = self.f_collided.to_sparse()
            log_VRAM("SIMULATION_step_after_store_f_collided")

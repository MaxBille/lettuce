...
matplotlib.rcParams.update({'lines.linestyle': '--', 'lines.markeredgewidth': 0.7, 'lines.markersize': 3})
...
plt.setp(lines, ls="", marker="x", color="tab:red", label="Roshko 1953 [exp.]")
...
plt.setp(lines, ls="", marker="v", color="tab:olive", label="Williamson 1989 [num.]")
...
hwbb_bgk = plt.plot(data[0], data[1], ls="--", lw=0.7, marker=".", color="tab:orange", label="HWBB BGK")
ibb_bgk = plt.plot(data[0], data[2], ls="--", lw=0.7, marker=".", color="tab:green", label="IBB BGK")
...
plt.savefig(folder+"/plots/"+name+"_v02.png")
plt.show()

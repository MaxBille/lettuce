...
matplotlib.rcParams.update({'lines.linestyle': '--', 'lines.markeredgewidth': 1, 'lines.markersize': 3})
...
plt.setp(lines_tritton, ls="", marker="v", color="tab:purple", label="Tritton 1959 [exp.]")

lines_relf = plt.plot(data_relf[:,0], data_relf[:,1])
plt.setp(lines_relf, ls="", marker="^", color="tab:cyan", label="Relf 1913 [exp.]")
...
hwbb_bgk = plt.plot(data[0], data[1], marker=".", color="tab:orange", label="HWBB BGK")
ibb_bgk = plt.plot(data[0], data[2], marker=".", color="tab:green", label="IBB BGK")

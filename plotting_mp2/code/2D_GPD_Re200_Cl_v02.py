...
#fwbb_reg = plt.semilogx(data[0], data[2], marker="^", color="tab:blue", label="FWBB REG")
...
#hwbb_reg = plt.semilogx(data[0], data[5], marker="^", color="tab:orange", label="HWBB REG")
...
#ibb_reg = plt.semilogx(data[0], data[8], marker="^", color="tab:green", label="IBB REG")
...
#plt.ylim([0.18,0.21])
plt.grid()
#plt.title("Widerstandsbeiwert $C_{L}$ in Abhängigkeit der Domänenbreite in Durchmessern (DpY), für Re = 200", wrap=True)
...
plt.xlabel("GPD")
plt.ylabel(r"$|C_{L} - \overline{C_{L,GPD128}}|$")
#plt.xlim([9,61])
#plt.ylim([0.18,0.21])
plt.grid()
#plt.title("Widerstandsbeiwert $C_{L}$ in Abhängigkeit der Domänenbreite in Durchmessern (DpY), für Re = 200", wrap=True)

...
for i in gpds:
    ...
    MASK = lattice.convert_to_numpy(sim._boundaries[2].mask)  # obstacle_mask


    if lattice.D == 2:
        nx, ny = MASK.shape  # Anzahl x-Punkte, Anzahl y-Punkte (Skalar), (der gesamten Simulationsdomain)

        rand_mask = np.zeros((nx, ny),dtype=bool)  # Randpunkte, die es gibt
        f_rand_mask = np.zeros((lattice.Q, nx, ny), dtype=bool)
    ...
    a,b = np.where(rand_mask)  # alle x und y Koorinaten, von Randknoten der Obstacle Mask, also Punkten, die mindestens einen Link zu Fluidknoten haben
    a_pos=sum(a)/len(a)  # x_Koordinate des Kreis-Zentrums
    b_pos=sum(b)/len(b)  # y-Koordiante des Kreis-Zentrums

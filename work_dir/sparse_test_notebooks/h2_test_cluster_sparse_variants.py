...
args["logVRAM"] = False
args["variant"] = "h2_clone_inplace"
output_data = False # output/log parameters, observables and vtk or vti (if output_vtk=True)
        ...
        def step(self, num_steps):
            ...
            for _ in range(num_steps):  # simulate num_step timesteps
               ...
               # torch.cuda.empty_cache()
                #torch.cuda.synchronize(lattice.device)
                print("(!) using h2-sim, step", _)
...
elif variant == "h2_clone":
    print("using IBB1 from h2_clone")
    class InterpolatedBounceBackBoundary:
    ...
    class Simulation:
        ...
        def step(self, num_steps):
            ...
            for _ in range(num_steps):  # simulate num_step timesteps
                ...
                if self.store_f_collided:
                    if self.lattice.D == 2:
                        self.f_collided_lt[:,0] = torch.clone(self.f[self._boundaries[-1].f_mask_sequential_lt[:, 0],  # q
                                                        self._boundaries[-1].f_mask_sequential_lt[:, 1],  # x
                                                        self._boundaries[-1].f_mask_sequential_lt[:, 2]])  # y
                        ...
                        self.f_collided_gt[:,0] = torch.clone(self.f[self._boundaries[-1].f_mask_sequential_gt[:, 0],  # q
                                                        self._boundaries[-1].f_mask_sequential_gt[:, 1],  # x
                                                        self._boundaries[-1].f_mask_sequential_gt[:, 2]])  # y
...
if calculate_force_coefficients:
    ...
    Dragreport = ObservableReporter(DragObservable, #interval=int(flow.units.convert_time_to_lu(T_target/1000)),
    ...
    Liftreport = ObservableReporter(LiftObservable, #interval=int(flow.units.convert_time_to_lu(T_target/1000)),
                                       out=None)
...
print("tensor f has grad:", sim.f.requires_grad)

#from occ_ad import OCC_has_ad, make_gp_Pnt, make_gp_Vec, torch_float_to_standard_adouble
import torch
#if OCC_has_ad:
...
def build_terrain_2D(surface_data, depth=1.):
    ...
    for x, y, *_ in surface_data:
        polygon_builder.Add(gp_Pnt(x.item(), y.item(), z))

    # close the polygon
    ymin = surface_data[:, 1].min().item()
    polygon_builder.Add(gp_Pnt(surface_data[-1, 0].item(), ymin - 10., z))
    polygon_builder.Add(gp_Pnt(surface_data[0, 0].item(), ymin - 10., z))
...
def build_house_max(surface_data, minz: float = 0., maxz: float = 1.):
    """
    ...
    for x, y in surface_data:
        polygon_builder.Add(gp_Pnt(x, y, minz))
    ...
    if searcher.Found():
        ...
        face_builder = BRepBuilderAPI_MakeFace(polygon)
    surroundings = BRepPrimAPI_MakePrism(face_builder.Shape(), gp_Vec(0., 0., maxz-minz))
    return surroundings.Shape()
...
def build_house(pos_x, pos_y, height, width, rel_roof_dx, rel_roof_dy, smooth=False, depth: float = 1., dim: int = 2,
    ...
    front_roof = gp_Pnt(pos_x, pos_y + height, minz)
    center_roof = gp_Pnt(pos_x + rel_roof_dx * width, pos_y + (1 + rel_roof_dy) * height, minz)
    back_roof = gp_Pnt(pos_x + width, pos_y + height, minz)
    ...
    back_house = gp_Pnt(pos_x + width, pos_y, minz)
    back = BRepBuilderAPI_MakeEdge(back_roof, back_house).Edge()

    front_house = gp_Pnt(pos_x, pos_y, minz)

...
from helpterCodePS.obstacleFunctions import overlap_solids, collect_solid_boundary_data, calculate_mask, makeGrid
from helpterCodePS.geometry import is_point_inside_solid, intersect_boundary_with_ray, extract_faces
   ...
   # boundary: LettuceBoundary
    ...
    def get_boundary(self):
        ...
        if isinstance(self.occ_object, torch.Tensor):
            ...
            if self.boundary_type is InterpolatedBounceBackBoundary_occ:
    ...
    def calculate_points_inside(self):
        ...
        if self.points_inside_known:
            ...
            return
        self.collision_data = calculate_mask(self.occ_object, torch.tensor(np.array(self.grid), device=self.lattice.device), self.name, self.collision_data)
        self.points_inside_known = True
        return

    def collect_collision_data(self):
        if self.boundary_type is not InterpolatedBounceBackBoundary_occ:
...
class ObstacleSurface:
    ...
    def initial_solution(self, x: torch.Tensor):
        ...
        if type(x[0]) is not torch.Tensor:
            x = torch.tensor(np.array(x), device=self.lattice.device)
        p = torch.zeros_like(x[0], dtype=self.lattice.dtype)[None, ...]
        y = torch.tensor(self.grid[1], device=self.lattice.device) #self.grid[1]
    ...
    def initialize_object_boundaries(self):
        ...
        if not self.all_fwbb:
            for boundary_type in [InterpolatedBounceBackBoundary_occ, PartiallySaturatedBoundary, BounceBackBoundary]:
                ...
                if boundary_list:
                    if boundary_type is InterpolatedBounceBackBoundary_occ:
    ...
    def overlap_all_solid_masks(self):
        ...
        for boundary_object in boundaries_list:
            self._solid_mask = self.solid_mask | boundary_object.solid_mask.to(self.lattice.device)
        time1 = time.time() - time0

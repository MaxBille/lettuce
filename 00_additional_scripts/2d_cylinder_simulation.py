##################################################
#PYTHON SCRIPT TO RUN AN OBSTACLE FLOW SIMULATION IN LETTUCE
# - 2D cylinder obstacle
# - takes simulation parameters as arguments
# - writes output to files
# - can be run on cluster via "cluster_job_script.sh"
# 
# ARGUMENTS:
# - Reynolds number, number of timesteps, gridpoints per diameter, diameters per domain width, time T to simulate (PU), collision operator
# - see #ARGUMENT PARSING below
# 
# PARAMETERS:
# - set below #PARAMETERS if not set or default via ARGUMENTS
# - Reynolds number, Mach number, number of timesteps, cylinder diameter (PU), inflow mean velocity (PU), resolution & domain size (GPD, DpY), time T to simulate (PU), initial velocity field, perturbation ON/OFF, channel flow ON/OFF, cylinder obstacle ON/OFF, Bounce-Back Boundary Algorithm (halfway/fullway), output settings (observables, vtk/vti)
# - set output names under # DATA OUTPUT SETTINGS
#
# OUTPUTS:
# - outputs written if vtk_out == TRUE and output_save == TRUE
# - filename-base specified below
# - short version of output and stats prior and after simulation are printed to stdout
# - creats timeseries and plots of drag- and liftcoefficient
# - calculates Strouhal number
# - creates list of all tensors and their memory usage
#
# SIM SETUPT and RUN:
# - simulation is set up: 
#    - lattice-object, 
#    - flow-object of Obstaclemax-Class
#    - obstacle definition under #OBSTACLE
#    - collision-object,
#    - simulation-object
# - simulation is run
# - outputs are written to stdout (some) and files (all). See # OUTPUTS
#
# FURTHER COMMENTS AND DESCRIPTIONS BELOW
...
if domain_width_in_D % 2 == 0 and gridpoints_per_diameter % 2 != 0:
    ...
    print("(!) domain_width_in_D (DpY) is even, gridpoints_per_diameter will be "+str(gridpoints_per_diameter)+". Use odd domain_width_in_D (DpY) to enable use of odd GPD!")
...
perturb_init = True   # setup initial solution with asymmetry to trigger vKarman vortex street for Re>46
bb_wall = False    # lateral walls on/off: True= bounce-back-walls and paraboplic velocity inlet, False= periodic BC and uniform velocity inlet
cylinder = True    # Objekt: True = cylinder, False = no obstascle
halfway = True    # BounceBackBoundary-Algorithm: True=Halfway, False=Fullway
drag_out = True    # report drag_coefficient
lift_out = True    # report lift_coefficient
vtk_fps=10    # FramesPerSecond (PU) for vtk-output
vtk_out=True   # vtk-output. is overwritten by output_save=False (see below)

gridpoints = gridpoints_per_diameter**2*domain_length_in_D*domain_width_in_D  # calc. total number of gridpoints

##################################################
# DATA OUTPUT SETTINGS (observables, stats and vtk)

output_save = True  # output/log parameters, observables and vtk or vti (if vtk_out=True)

# naming: specify batch number, batch name, version name/number and parameters to put in directory- and datafile-names
batch = "_batchX_NAME"
version = "__GPD"+str(gridpoints_per_diameter)+"_DpY"+str(domain_width_in_D)

if output_save:  # toggle output of parameters, observables and vti/vtk files
    # (see above) vtk_out = True    # vtk-reporter outputs vtk or vti files for visualization in Paraview
    ...
    dir_name = "/data_" + str(timestamp) + batch + version  # create directory name for all outputs to be saved in
...
xLU, yLU = np.meshgrid(*xyz, indexing='ij')  # meshgrid of x-, y- (and z-) indexes
...
if drag_out == True:
    DragObservable = lt.DragCoefficient(lattice,flow,sim,area=setup_diameter)  # create observable // ! area A=2*r is in PU

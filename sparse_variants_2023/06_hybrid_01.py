                ...
                for i in range(0, self.lattice.Q):  # for all stencil-directions c_i (lattice.stencil.e in lettuce)
                    ...
                    try:  # try in case the neighboring cell does not exist (= an f pointing out of the simulation domain)
                        if not mask[a[p] + self.lattice.stencil.e[i, 0] - border[0] * nx,
                            ...
                            self.f_mask_sequential.append([self.lattice.stencil.opposite[i],a[p] + self.lattice.stencil.e[i, 0] - border[0] * nx,b[p] + self.lattice.stencil.e[i, 1] - border[1] * ny])
...
#        self.d = self.lattice.convert_to_tensor(self.d)
    ...
    def __call__(self, f, f_collided):
        ...
        #     log_VRAM("IBB1_call_f_tmp_created")
        ...
        # for i in range(self.d_sequential.shape[0]):
        #         # 0:q, 1:x, 2:y, 3:z 4:d
        #         q = int(self.d_sequential[i,0])
        #         x = int(self.d_sequential[i,1])
        #         y = int(self.d_sequential[i,2])
        #         z = int(self.d_sequential[i,3])
        #         d = int(self.d_sequential[i,4])
        #         if d <= 0.5:
        #             f[self.lattice.stencil.opposite[q],x,y,z] = 2*d*f_collided[q,x,y,z]+(1-2*d*f[q,x,y,z])
        #         else:
        #             f[self.lattice.stencil.opposite[q],x,y,z] = (1/(2*d))*f_collided[q,x,y,z]+(1-1/(2*d))*f_collided[self.lattice.stencil.opposite[q]]

        # if d <= 0.5
        f[self.opposite_tensor[self.f_mask_sequential_lt[:,0]],
          self.f_mask_sequential_lt[:,1],
          self.f_mask_sequential_lt[:,2],
          self.f_mask_sequential_lt[:,3]] = 2*self.d_sequential_lt*f_collided.to_dense()[self.f_mask_sequential_lt[:,0],
                                                                                              self.f_mask_sequential_lt[:,1],
                                                                                              self.f_mask_sequential_lt[:,2],
        ...
        f[self.opposite_tensor[self.f_mask_sequential_gt[:,0]],
          ...
          self.f_mask_sequential_gt[:,3]] = (1/(2*self.d_sequential_gt))*f_collided.to_dense()[self.f_mask_sequential_gt[:,0],
                                                                                         self.f_mask_sequential_gt[:,1],
                                                                                         self.f_mask_sequential_gt[:,2],
                                                                                         self.f_mask_sequential_gt[:,3]]\
                                            +(1-1/(2*self.d_sequential_gt))*f_collided.to_dense()[self.opposite_tensor[self.f_mask_sequential_gt[:,0]],
                                                                                            self.f_mask_sequential_gt[:,1],
                                                                                            self.f_mask_sequential_gt[:,2],
                                                                                            self.f_mask_sequential_gt[:,3]]
    ...
    def calc_force_on_boundary(self, f_bounced, f_collided):
        ...
        # log_VRAM("IBB1_force_tmp_created")
        # self.force_sum = torch.einsum('i..., id -> d', tmp, self.lattice.e)  # CALCULATE FORCE / v3.0 - M.Bille: dx_lu = dt_lu is allways 1 (!)

        # self.force_sum = torch.zeros_like(self.force_sum)
        # for i in range(self.d_sequential.shape[0]):
        #     q = int(self.d_sequential[i,0])
        #     x = int(self.d_sequential[i,1])
        #     y = int(self.d_sequential[i,2])
        #     z = int(self.d_sequential[i,3])
        #     self.force_sum += self.lattice.convert_to_tensor([self.lattice.e[q,0]*(f_collided[q,x,y,z]+f_bounced[self.lattice.stencil.opposite[q],x,y,z]),
        #                        self.lattice.e[q,1]*(f_collided[q,x,y,z]+f_bounced[self.lattice.stencil.opposite[q],x,y,z]),
        #                        self.lattice.e[q,2]*(f_collided[q,x,y,z]+f_bounced[self.lattice.stencil.opposite[q],x,y,z])])
        ...
        self.force_sum = torch.einsum('i..., id -> d', f_collided.to_dense()[self.f_mask_sequential_lt[:,0],
                                                            ...
                                                            + f_bounced[self.opposite_tensor[self.f_mask_sequential_lt[:,0]],
                                                                                             self.f_mask_sequential_lt[:,1],
                                                                                             self.f_mask_sequential_lt[:,2],
                                                                                             self.f_mask_sequential_lt[:,3]], self.lattice.e[self.f_mask_sequential_lt[:,0]]) \
                         + torch.einsum('i..., id -> d', f_collided.to_dense()[self.f_mask_sequential_gt[:,0],
                                                            ...
                                                            + f_bounced[self.opposite_tensor[self.f_mask_sequential_gt[:,0]],
                                                                                             self.f_mask_sequential_gt[:,1],
                                                                                             self.f_mask_sequential_gt[:,2],
                                                                                             self.f_mask_sequential_gt[:,3]], self.lattice.e[self.f_mask_sequential_gt[:,0]])

        # tmp = torch.where(self.f_mask, f_collided.to_dense() + f_bounced[self.lattice.stencil.opposite], torch.zeros_like(f_bounced))
        # force_sum2 = torch.einsum('i..., id -> d', tmp, self.lattice.e)
        # print("f_sum - f_sum_old", self.force_sum - force_sum2)
        # force = self.lattice.e[self.f_mask_sequential_lt[:,0]]*(f_collided.to_dense()[self.f_mask_sequential_lt[:,0],
        #                                                                           self.f_mask_sequential_lt[:,1],
        #                                                                           self.f_mask_sequential_lt[:,2],
        #                                                                           self.f_mask_sequential_lt[:,3]]\
        #                                                     + f_bounced[self.opposite_tensor[self.f_mask_sequential_lt[:,0]],
        ...
        #                                                                                      self.f_mask_sequential_lt[:,2],
        #                                                                                      self.f_mask_sequential_lt[:,3]] )\
        #         + self.lattice.e[self.f_mask_sequential_gt[:,0]]*(f_collided.to_dense()[self.f_mask_sequential_gt[:,0],
        #                                                                            self.f_mask_sequential_gt[:,1],
        #                                                                            self.f_mask_sequential_gt[:,2],
        #                                                                            self.f_mask_sequential_gt[:,3]]\
        #                                                    + f_bounced[self.opposite_tensor[self.f_mask_sequential_gt[:,0]],
        ...
        #                                                                self.f_mask_sequential_gt[:,2],
        #                                                                self.f_mask_sequential_gt[:,3]] )
        # self.force_sum = torch.sum(force, dim=1)
...
class Simulation:
    ...
    def __init__(self, flow, lattice, collision, streaming):
        ...
        if self.store_f_collided:
            ...
            # log_VRAM("SIMULATION_init_after_copy_f_collided")
            # self.f_collided = deepcopy(f_collided_tmp).to_sparse()
            # del f_collided_tmp
            ...
            log_VRAM("SIMULATION_init_after_where_fmask")
            self.f_collided = deepcopy(torch.sparse_coo_tensor(indices=self.fc_index,
                                                      values=self.f[self.fc_index[0], self.fc_index[1], self.fc_index[2], self.fc_index[3]],
                                                      size=self.f.size()))
        log_VRAM("SIMULATION_init_after_store_f_collided (sparse)")
    ...
    def step(self, num_steps):
        ...
        for _ in range(num_steps):  # simulate num_step timesteps
            ...
            if self.store_f_collided:
                #self.f_collided = deepcopy(self.f)
                # f_collided_tmp = torch.where(self._boundaries[-1].f_mask+self._boundaries[-1].f_mask[self.lattice.stencil.opposite], self.f, torch.zeros_like(self.f))
                # log_VRAM("SIMULATION_step_after_copy_f_collided")
                # self.f_collided = deepcopy(f_collided_tmp).to_sparse()
                # del f_collided_tmp
                self.f_collided = deepcopy(torch.sparse_coo_tensor(indices=self.fc_index,
                                                      values=self.f[self.fc_index[0], self.fc_index[1], self.fc_index[2], self.fc_index[3]],
                                                      size=self.f.size()))
            log_VRAM("SIMULATION_step_after_store_f_collided (sparse)")

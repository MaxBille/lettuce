...
for i in gpds:
   ...
   # collect data on radii on the outside of the cylinder (all gridpoints within the cylinder, that have at least one stencil-link to a fluid node outside the cylinder)
    ...
    if False:
        ...
        plt.savefig(folder+"/plots/roundness/other_masks/maskGPD" + str(gridpoints_per_diameter) + ".png")
        plt.show()
...
if True:
    ...
    plt.savefig(folder+"/plots/roundness/Histogram.png")
    plt.show()
    ...
    ax.xaxis.set_minor_locator(AutoMinorLocator())
    plt.xticks([0.85,0.9,0.95,1.0])
    plt.savefig(folder+"/plots/roundness/Histogram_q.png")
    plt.show()
...
if False:
    ...
    plt.xlabel("GPD")
    plt.ylabel("$r_{rel}$")
    plt.savefig(folder+"/plots/roundness/mittlererRadius.png")
    plt.show()
